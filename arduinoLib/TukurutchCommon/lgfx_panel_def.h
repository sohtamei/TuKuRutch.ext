enum {
	...
	LCDTYPE_ESP32S3GEEK = 37,
	LCDTYPE_PICO_CAMA = 38,
...
class LGFX_PICO_CAMA : public lgfx::LGFX_Device
{
	lgfx::Panel_ST7789	_panel_instance;
#if defined(ESP32)
	lgfx::Bus_SPI		_bus_spi;			// SPIバスのインスタンス
...
#endif
public:
	LGFX_PICO_CAMA(int lcdType, uint8_t *config_buf, int config_size)
	{
		nvscfg_spi_t nvs = { .sclk=10, .mosi=11, .miso=-1, .dc= 9, .cs=13, .rst= 7, .busy=-1, .bl=-1, };
...
// ESP32S3-GEEK
class LGFX_ESP32S3GEEK : public lgfx::LGFX_Device
{
	lgfx::Panel_ST7789	_panel_instance;
#if defined(ESP32)
	lgfx::Bus_SPI		_bus_spi;			// SPIバスのインスタンス
...
#endif
public:
	LGFX_ESP32S3GEEK(int lcdType, uint8_t *config_buf, int config_size)
	{
		nvscfg_spi_t nvs = { .sclk=12, .mosi=11, .miso=-1, .dc= 8, .cs=10, .rst= 9, .busy=-1, .bl= 7, };

		spi_host_device_t spi_ch = SPI3_HOST;  // SDはSPI2_HOSTでないと動かない
		if(spi_ch < 0) return;
